...
typedef void(^HelloTimerChangeBlock)(NSString *day, NSString *hour, NSString *minute, NSString *second, NSString *ms);
typedef void(^HelloTimerFinishBlock)(NSString *identifier);
typedef void(^HelloTimerPauseBlock)(NSString *identifier);



/** 倒计时变化通知类型 */
typedef NS_ENUM(NSInteger, HelloTimerSecondChangeNFType) {
    HelloTimerSecondChangeNFTypeNone = 0,
    HelloTimerSecondChangeNFTypeMS,        //每100ms(毫秒) 发出一次
    HelloTimerSecondChangeNFTypeSecond,    //每1s(秒) 发出一次

};

@interface HelloTimer : NSObject
