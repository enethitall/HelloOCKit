...
static NSString *const HelloPopViewLogTitle = @"HelloPopView日志 ---> :";
static HelloPopViewLogStyle _logStyle;

UIWindow *hello_Window(void) {
...
@interface HelloPopViewManager : NSObject

/** 单例 */
HelloPopViewManager *HelloPopViewM(void);

@end

@interface HelloPopViewManager ()
...
/** 储存待移除的popView  */
@property (nonatomic,strong) NSHashTable <HelloPopView *> *removeList;
/** 内存信息View */
@property (nonatomic,strong) UILabel *infoView;

@end

@implementation HelloPopViewManager

static HelloPopViewManager *_instance;

HelloPopViewManager *HelloPopViewM(void) {
    return [HelloPopViewManager sharedInstance];
}
...
+ (NSArray *)getAllPopView {
    return HelloPopViewM().popViewMarr;
}
/** 获取当前页面所有popView */
+ (NSArray *)getAllPopViewForParentView:(UIView *)parentView {
    NSMutableArray *mArr = HelloPopViewM().popViewMarr;
    NSMutableArray *resMarr = [NSMutableArray array];
    for (id obj in mArr) {
        HelloPopView *popView = (HelloPopView *)obj;
...
+ (NSArray *)getAllPopViewForPopView:(HelloPopView *)popView {
    NSArray *mArr = [self getAllPopViewForParentView:popView.parentView];
    NSMutableArray *resMarr = [NSMutableArray array];
    for (id obj in mArr) {
        HelloPopView *tPopView = (HelloPopView *)obj;
...
/** 读取popView */
+ (HelloPopView *)getPopViewForKey:(NSString *)key {
...
+ (void)savePopView:(HelloPopView *)popView {
    [HelloPopViewM().popViewMarr addObject:popView];
    
    //优先级排序
    [self sortingArr];
    
    if (_logStyle & HelloPopViewLogStyleWindow) {
        [self setInfoData];
    }
    if (_logStyle & HelloPopViewLogStyleConsole) {
        [self setConsoleLog];
    }
}

/** 移除popView */
+ (void)removePopView:(HelloPopView *)popView {
    if (!popView) { return; }
    NSArray *arr = HelloPopViewM().popViewMarr;
    for (id obj in arr) {
        HelloPopView *tPopView = (HelloPopView *)obj;
        
        if ([tPopView isEqual:popView]) {
            [tPopView dismissWithStyle:HelloDismissStyleNO];
            break;
        }
    }
...
/** 移除popView */
+ (void)removePopViewForKey:(NSString *)key complete:(Hello_Block_Void)complete {
    if (key.length<=0) {
...
/** 移除所有popView */
+ (void)removeAllPopViewComplete:(Hello_Block_Void)complete {
    NSMutableArray *arr = HelloPopViewM().popViewMarr;
    if (arr.count <= 0) {
        complete ? complete() : nil;
        return;
    }
    
    NSArray *popViewMarr = [NSArray arrayWithArray:HelloPopViewM().popViewMarr];
...
+ (void)removeLastPopViewComplete:(Hello_Block_Void)complete {
    NSPointerArray *showList =  HelloPopViewM().showList;
    ...
    for (NSInteger i = showList.count - 1; i >= 0; i --) {
        HelloPopView *popView = [showList pointerAtIndex:i];
        if (popView) {
...
+ (void)setInfoData {
    HelloPopViewM().infoView.text = [NSString stringWithFormat:@"S:%zd R:%zd",HelloPopViewM().popViewMarr.count,HelloPopViewM().removeList.allObjects.count];
}

+ (void)setConsoleLog {
    HelloLog(@"%@ S:%zd个 R:%zd个",HelloPopViewLogTitle,HelloPopViewM().popViewMarr.count,HelloPopViewM().removeList.allObjects.count);
}
...
+ (void)sortingArr {
    ...
    for (int i = 0; i < arr.count; i++) {
        for (int j = i+1; j < arr.count; j++) {
            HelloPopView *iPopView = (HelloPopView *)arr[i];
            
            HelloPopView *jPopView = (HelloPopView *)arr[j];
...
/** 转移popView到待移除队列 */
+ (void)transferredToRemoveQueueWithPopView:(HelloPopView *)popView {
    NSArray *popViewMarr = HelloPopViewM().popViewMarr;
    
    [popViewMarr enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL * stop) {
        HelloPopView *tPopView = (HelloPopView *)obj;
        
        if ([popView isEqual:tPopView]) {
            [HelloPopViewM().removeList addObject:obj];
            [HelloPopViewM().popViewMarr removeObject:obj];
        }
    }];
}

+ (void)destroyInRemoveQueue:(HelloPopView *)popView {
    if (HelloPopViewM().removeList.allObjects.count <= 0) {
...
- (UILabel *)infoView {
    ...
    _infoView.frame = CGRectMake(hl_ScreenWidth()-70, hl_IsIphoneX()? 40:20, 60, 10);
    _infoView.layer.cornerRadius = 1;
    _infoView.layer.masksToBounds = YES;
    UIView *superView = hello_Window();
...
- (NSHashTable<HelloPopView *> *)removeList {
...
static const NSTimeInterval HelloPopViewDefaultDuration = -1.0f;
// 角度转弧度
#define LST_DEGREES_TO_RADIANS(angle) ((angle) / 180.0 * M_PI)


@interface HelloPopViewBgView : UIView
...
@implementation HelloPopViewBgView
...
@interface HelloPopView () <UIGestureRecognizerDelegate>

/** 弹窗容器 默认是app UIWindow 可指定view作为容器 */
@property (nonatomic, weak) UIView *container;
/** 背景层 */
@property (nonatomic, strong) HelloPopViewBgView *backgroundView;
/** 自定义视图 */
@property (nonatomic,strong) UIView *customView;
/** 规避键盘偏移量 */
@property (nonatomic, assign) CGFloat avoidKeyboardOffset;
/** 代理池 */
@property (nonatomic, strong) NSHashTable <id<HelloPopViewProtocol>> *delegates;
/** 背景点击手势 */
...
- (void)initSubViews {
    ...
    _bgAlpha = 0.25;
    _popStyle = HelloPopStyleFade;
    _dismissStyle = HelloDismissStyleFade;
    _popDuration = HelloPopViewDefaultDuration;
    _dismissDuration = HelloPopViewDefaultDuration;
    _hemStyle = HelloHemStyleCenter;
    _adjustX = 0;
    _adjustY = 0;
    _isClickFeedback = NO;
    ...
    //拖拽相关属性初始化
    _dragStyle = HelloDragStyleNO;
    _dragDismissStyle = HelloDismissStyleNO;
    _dragDistance = 0.0f;
    _dragReboundTime = 0.25;
    _dragDismissDuration = HelloPopViewDefaultDuration;
...
+ (nullable instancetype)initWithCustomView:(UIView *_Nonnull)customView {
    return [self initWithCustomView:customView popStyle:HelloPopStyleNO dismissStyle:HelloDismissStyleNO];
}


+ (nullable instancetype)initWithCustomView:(UIView *_Nonnull)customView
                                   popStyle:(HelloPopStyle)popStyle
                               dismissStyle:(HelloDismissStyle)dismissStyle {
...
+ (nullable instancetype)initWithCustomView:(UIView *_Nonnull)customView
    ...
    if (parentView) {
        ...
        popViewFrame =  CGRectMake(0, 0, hl_ScreenWidth(), hl_ScreenHeight());
    }
    
    HelloPopView *popView = [[HelloPopView alloc] initWithFrame:popViewFrame];
    popView.backgroundColor = [UIColor clearColor];

    popView.container = parentView? parentView : hello_Window();

    popView.customView = customView;
    popView.backgroundView = [[HelloPopViewBgView alloc] initWithFrame:popView.bounds];
...
- (void)dealloc {
    ...
    //从待移除队列中销毁
    [HelloPopViewManager destroyInRemoveQueue:self];
    [self lst_PopViewDidDismissForPopView:self];
    
    for (Hello_Block_Void block in _popViewDidDismissBlocks.copy) {
        block ? block() : nil;
    }
...
- (void)removeFromSuperview {
    ...
    [HelloPopViewManager transferredToRemoveQueueWithPopView:self];
}
...
#pragma mark - ***** 代理方法 *****
- (void)lst_PopViewBgClickForPopView:(HelloPopView *)popView {
    for (id<HelloPopViewProtocol> delegate in _delegates.copy) {
        if ([delegate respondsToSelector:_cmd]) {
            [delegate hl_PopViewBgClickForPopView:popView];
        }
    }
}

- (void)lst_PopViewBgLongPressForPopView:(HelloPopView *)popView {
    for (id<HelloPopViewProtocol> delegate in _delegates.copy) {
        if ([delegate respondsToSelector:_cmd]) {
            [delegate hl_PopViewBgLongPressForPopView:popView];
        }
    }
}

- (void)lst_PopViewWillPopForPopView:(HelloPopView *)popView {
    for (id<HelloPopViewProtocol> delegate in _delegates.copy) {
        if ([delegate respondsToSelector:_cmd]) {
            [delegate hl_PopViewWillPopForPopView:popView];
        }
    }
}

- (void)lst_PopViewDidPopForPopView:(HelloPopView *)popView {
    for (id<HelloPopViewProtocol> delegate in _delegates.copy) {
        if ([delegate respondsToSelector:_cmd]) {
            [delegate hl_PopViewDidPopForPopView:popView];
        }
    }
}

- (void)lst_PopViewCountDownForPopView:(HelloPopView *)popView forCountDown:(NSTimeInterval)timeInterval {
    for (id<HelloPopViewProtocol> delegate in _delegates.copy) {
        if ([delegate respondsToSelector:_cmd]) {
            [delegate hl_PopViewCountDownForPopView:popView forCountDown:timeInterval];
        }
    }
}

- (void)lst_PopViewCountDownFinishForPopView:(HelloPopView *)popView {
    for (id<HelloPopViewProtocol> delegate in _delegates.copy) {
        if ([delegate respondsToSelector:_cmd]) {
            [delegate hl_PopViewCountDownFinishForPopView:popView];
        }
    }
}

- (void)lst_PopViewWillDismissForPopView:(HelloPopView *)popView {
    for (id<HelloPopViewProtocol> delegate in _delegates.copy) {
        if ([delegate respondsToSelector:_cmd]) {
            [delegate hl_PopViewWillDismissForPopView:popView];
        }
    }
}

- (void)lst_PopViewDidDismissForPopView:(HelloPopView *)popView {
    for (id<HelloPopViewProtocol> delegate in _delegates.copy) {
        if ([delegate respondsToSelector:_cmd]) {
            [delegate hl_PopViewDidDismissForPopView:popView];
        }
    }
}
...
- (void)setCustomViewFrameWithHeight:(CGFloat)height {
    ...
    switch (self.hemStyle) {
        case HelloHemStyleTop ://贴顶
        {
            self.customView.hl_x = _backgroundView.hl_centerX - _customView.hl_size.width*0.5 + _adjustX;
            self.customView.hl_y = _adjustY;
            changeY = _adjustY;
        }
            break;
        case HelloHemStyleLeft ://贴左
        {
            self.customView.hl_x = _adjustX;
            self.customView.hl_y = _backgroundView.hl_centerY - _customView.hl_size.height*0.5 + _adjustY;
            changeY = height*0.5;
        }
            break;
        case HelloHemStyleBottom ://贴底
        {
            self.customView.hl_x = _backgroundView.hl_centerX - _customView.hl_size.width*0.5 + _adjustX;
            [self.customView layoutIfNeeded];
            self.customView.hl_y = _backgroundView.hl_height - _customView.hl_height + _adjustY;
            changeY = height;
        }
            break;
        case HelloHemStyleRight ://贴右
        {
            self.customView.hl_x = _backgroundView.hl_width - _customView.hl_width + _adjustX;
            self.customView.hl_y = _backgroundView.hl_centerY - _customView.hl_size.height*0.5 + _adjustY;
            changeY = height*0.5;
        }
            break;
        case HelloHemStyleTopLeft :///贴顶和左
        {
            self.customView.hl_x = _adjustX;
            self.customView.hl_y = _adjustY;
            changeY = _adjustY;
            ...
            break;
        case HelloHemStyleBottomLeft ://贴底和左
        {
            self.customView.hl_x = _adjustX;
            [self.customView layoutIfNeeded];
            self.customView.hl_y = _backgroundView.hl_height - _customView.hl_height + _adjustY;
            ...
            break;
        case HelloHemStyleBottomRight ://贴底和右
        {
            self.customView.hl_x = _backgroundView.hl_width - _customView.hl_width + _adjustX;
            [self.customView layoutIfNeeded];
            self.customView.hl_y = _backgroundView.hl_height - _customView.hl_height + _adjustY;
            ...
            break;
        case HelloHemStyleTopRight ://贴顶和右
        {
            self.customView.hl_x = _backgroundView.hl_width - _customView.hl_width + _adjustX;
            self.customView.hl_y = _adjustY;
            changeY = _adjustY;
    ...
    if (_isShowKeyboard && (originBottom > _keyboardY)) {//键盘已经显示
        CGFloat Y = _keyboardY - _customView.hl_height - _avoidKeyboardSpace;
        _customView.hl_y = Y;
        CGRect newFrame = _originFrame;
        newFrame.origin.y = newFrame.origin.y - changeY;
        newFrame.size = CGSizeMake(_originFrame.size.width, _customView.hl_height);
...
- (void)setHemStyle:(HelloHemStyle)hemStyle {
    _hemStyle = hemStyle;
}
...
- (void)setDelegate:(id<HelloPopViewProtocol>)delegate {
    if ([self.delegates containsObject:delegate]) return;
    [self.delegates addObject:delegate];
}

- (void)setDragDismissStyle:(HelloDismissStyle)dragDismissStyle {
...
- (void)popWithStyle:(HelloPopStyle)popStyle {
...
- (void)popWithStyle:(HelloPopStyle)popStyle duration:(NSTimeInterval)duration {
    [self popWithPopStyle:popStyle duration:duration isOutStack:NO];
}

- (void)popWithPopStyle:(HelloPopStyle)popStyle duration:(NSTimeInterval)duration isOutStack:(BOOL)isOutStack {
    ...
    if (self.isSingle) {//单显
        NSArray *popViewArr = [HelloPopViewManager getAllPopViewForPopView:self];
        for (id obj  in popViewArr) {//移除所有popView和移除定a时器
            HelloPopView *lastPopView = (HelloPopView *)obj;
            
            [lastPopView dismissWithDismissStyle:HelloDismissStyleNO duration:0.2 isRemove:YES];
        }
        startTimer = YES;
    }else {//多显
            ...
            if (popViewArr.count >= 1) {
                ...
                if (self.isStack) {//堆叠显示
                    ...
                    if (self.priority >= lastPopView.priority) {//置顶显示
                        ...
                        [lastPopView dismissWithDismissStyle:HelloDismissStyleFade duration:0.2 isRemove:NO];
                        startTimer = YES;
                    } else {//隐藏显示
                        if (!self.parentView) {
                            self.container = hello_Window();
                        }
                        [HelloPopViewManager savePopView:self];
                        return;
                    }
                }
            } else {
...
//    }
    if (!self.superview) {
        ...
        [HelloPopViewM().showList addPointer:(__bridge void * _Nullable)self];
    }
    ...
    //保存popView
    if (!isOutStack) { [HelloPopViewManager savePopView:self]; }
}

- (void)popAnimationWithPopStyle:(HelloPopStyle)popStyle duration:(NSTimeInterval)duration {
    
    HelloPopViewWK(self);
    if (popStyle == HelloPopStyleFade) {//渐变出现
        self.backgroundView.backgroundColor = [self getNewColorWith:self.bgColor alpha:0.0];
    ...
    } else if (popStyle == HelloPopStyleNO){//无动画
        self.backgroundView.backgroundColor = [self getNewColorWith:self.bgColor alpha:0.0];
...
- (void)hanlePopAnimationWithDuration:(NSTimeInterval)duration
    ...
    HelloPopViewWK(self);
    switch (popStyle) {
        case HelloPopStyleScale:// < 缩放动画，先放大，后恢复至原大小
        {
            ...
            break;
        case HelloPopStyleSmoothFromTop:
        case HelloPopStyleSmoothFromBottom:
        case HelloPopStyleSmoothFromLeft:
        case HelloPopStyleSmoothFromRight:
        case HelloPopStyleSpringFromTop:
        case HelloPopStyleSpringFromLeft:
        case HelloPopStyleSpringFromBottom:
        case HelloPopStyleSpringFromRight:
        {
            CGPoint startPosition = self.customView.layer.position;
            if (popStyle == HelloPopStyleSmoothFromTop || popStyle == HelloPopStyleSpringFromTop) {
                self.customView.layer.position = CGPointMake(startPosition.x, -_customView.hl_height*0.5);
                
            } else if (popStyle == HelloPopStyleSmoothFromLeft || popStyle == HelloPopStyleSpringFromLeft) {
                self.customView.layer.position = CGPointMake(-_customView.hl_width*0.5, startPosition.y);
                
            } else if (popStyle == HelloPopStyleSmoothFromBottom || popStyle == HelloPopStyleSpringFromBottom) {
                self.customView.layer.position = CGPointMake(startPosition.x, CGRectGetMaxY(self.frame) + _customView.hl_height*0.5);
                
            } else {
                self.customView.layer.position = CGPointMake(CGRectGetMaxX(self.frame) + _customView.hl_width*0.5 , startPosition.y);
            }
            
            CGFloat damping = 1.0;
            if (popStyle == HelloPopStyleSpringFromTop||
                popStyle == HelloPopStyleSpringFromLeft||
                popStyle == HelloPopStyleSpringFromBottom||
                popStyle == HelloPopStyleSpringFromRight) {
                damping = 0.65;
            }
            ...
            break;
        case HelloPopStyleCardDropFromLeft:
        case HelloPopStyleCardDropFromRight:
        {
            CGPoint startPosition = self.customView.layer.position;
            if (popStyle == HelloPopStyleCardDropFromLeft) {
                self.customView.layer.position = CGPointMake(startPosition.x * 1.0, -_customView.hl_height*0.5);
            ...
            [UIView animateWithDuration:duration*0.6 animations:^{
                wk_self.customView.layer.transform = CATransform3DMakeRotation(LST_DEGREES_TO_RADIANS((popStyle == HelloPopStyleCardDropFromRight) ? 5.5 : -5.5), 0, 0, 0);
            } completion:^(BOOL finished) {
                [UIView animateWithDuration:duration*0.2 animations:^{
                    wk_self.customView.transform = CGAffineTransformMakeRotation(LST_DEGREES_TO_RADIANS((popStyle == HelloPopStyleCardDropFromRight) ? -1.0 : 1.0));
                } completion:^(BOOL finished) {
                    [UIView animateWithDuration:duration*0.2 animations:^{
...
- (void)dismissWithStyle:(HelloDismissStyle)dismissStyle {
...
- (void)dismissWithStyle:(HelloDismissStyle)dismissStyle duration:(NSTimeInterval)duration  {
    [self dismissWithDismissStyle:dismissStyle duration:duration isRemove:YES];
}

- (void)dismissWithDismissStyle:(HelloDismissStyle)dismissStyle
    ...
    if (!self.isSingle && (isRemove && [HelloPopViewManager getAllPopViewForPopView:self].count >= 1)){//多显
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(resDuration * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            //popView出栈
            if (!self.isStack && [HelloPopViewManager getAllPopViewForPopView:self].count >= 1) {
                NSArray *popViewArr = [HelloPopViewManager getAllPopViewForPopView:self];
                id obj = popViewArr[popViewArr.count-1];
                HelloPopView *tPopView = (HelloPopView *)obj;
...
- (void)dismissAnimationWithDismissStyle:(HelloDismissStyle)dismissStyle duration:(NSTimeInterval)duration {
    
    HelloPopViewWK(self);
    if (dismissStyle == HelloPopStyleFade) {
        [UIView animateWithDuration:0.2 animations:^{
            wk_self.backgroundView.backgroundColor = [self getNewColorWith:self.bgColor alpha:0.0];
            wk_self.customView.alpha = 0.0f;
        }];
    }else if (dismissStyle == HelloPopStyleNO){
        wk_self.backgroundView.backgroundColor = [self getNewColorWith:self.bgColor alpha:0.0];
        wk_self.customView.alpha = 0.0f;
    }else {//有动画
...
- (void)hanleDismissAnimationWithDuration:(NSTimeInterval)duration
                         withDismissStyle:(HelloDismissStyle)dismissStyle {
    ...
    HelloPopViewWK(self);;
    switch (dismissStyle) {
        case HelloDismissStyleScale:
        {
            [self animationWithLayer:self.customView.layer duration:((0.2*duration)/0.8) values:@[@1.0, @0.66, @0.33, @0.01]];
        }
            break;
        case HelloDismissStyleSmoothToTop:
        case HelloDismissStyleSmoothToBottom:
        case HelloDismissStyleSmoothToLeft:
        case HelloDismissStyleSmoothToRight:
        {
            CGPoint startPosition = self.customView.layer.position;
            CGPoint endPosition = self.customView.layer.position;
            if (dismissStyle == HelloDismissStyleSmoothToTop) {
                endPosition = CGPointMake(startPosition.x, -(_customView.hl_height*0.5));
                
            } else if (dismissStyle == HelloDismissStyleSmoothToBottom) {
                endPosition = CGPointMake(startPosition.x, CGRectGetMaxY(self.frame) + _customView.hl_height*0.5);
                
            } else if (dismissStyle == HelloDismissStyleSmoothToLeft) {
                endPosition = CGPointMake(-_customView.hl_width*0.5, startPosition.y);
                
            } else {
                endPosition = CGPointMake(CGRectGetMaxX(self.frame) + _customView.hl_width*0.5, startPosition.y);
            }
            ...
            break;
        case HelloDismissStyleCardDropToLeft:
        case HelloDismissStyleCardDropToRight:
        {
            CGPoint startPosition = self.customView.layer.position;
            ...
            [UIView animateWithDuration:duration delay:0 usingSpringWithDamping:0.5 initialSpringVelocity:1.0 options:UIViewAnimationOptionCurveEaseIn animations:^{
                if (dismissStyle == HelloDismissStyleCardDropToLeft) {
            ...
            break;
        case HelloDismissStyleCardDropToTop:
        {
            CGPoint startPosition = self.customView.layer.position;
...
- (NSTimeInterval)getPopDuration:(NSTimeInterval)currDuration {
    ...
    if (_popStyle == HelloPopStyleNO) { return 0.0f; }
    
    if (_popStyle == HelloPopStyleFade) { return 0.2f; }
    
    if (currDuration<=0) { self.popDuration = HelloPopViewDefaultDuration; }
    
    if (self.popDuration == HelloPopViewDefaultDuration) {
        NSTimeInterval defaultDuration = 0.0f;

        if (_popStyle == HelloPopStyleScale) {
            defaultDuration = 0.3f;
        }
        if (_popStyle == HelloPopStyleSmoothFromTop ||
            _popStyle == HelloPopStyleSmoothFromLeft ||
            _popStyle == HelloPopStyleSmoothFromBottom ||
            _popStyle == HelloPopStyleSmoothFromRight ||
            _popStyle == HelloPopStyleSpringFromTop ||
            _popStyle == HelloPopStyleSpringFromLeft ||
            _popStyle == HelloPopStyleSpringFromBottom ||
            _popStyle == HelloPopStyleSpringFromRight ||
            _popStyle == HelloPopStyleCardDropFromLeft ||
            _popStyle == HelloPopStyleCardDropFromRight) {
...
- (NSTimeInterval)getDismissDuration:(NSTimeInterval)currDuration {
    ...
    if (_dismissStyle == HelloDismissStyleNO) { return 0.0f; }
    
    if (_dismissStyle == HelloDismissStyleFade) { return 0.2f; }
    
    if (self.dismissDuration == HelloPopViewDefaultDuration) {
        NSTimeInterval defaultDuration = 0.0f;
        if (_dismissStyle == HelloDismissStyleNO) {
            defaultDuration = 0.0f;
        }
        
        if (_dismissStyle == HelloDismissStyleScale) {
            defaultDuration = 0.3f;
        }
        if (_dismissStyle == HelloDismissStyleSmoothToTop ||
            _dismissStyle == HelloDismissStyleSmoothToBottom ||
            _dismissStyle == HelloDismissStyleSmoothToLeft ||
            _dismissStyle == HelloDismissStyleSmoothToRight ||
            _dismissStyle == HelloDismissStyleCardDropToLeft ||
            _dismissStyle == HelloDismissStyleCardDropToRight ||
            _dismissStyle == HelloDismissStyleCardDropToTop) {
...
- (NSTimeInterval)getDragDismissDuration {
    if (self.dragDismissDuration ==  HelloPopViewDefaultDuration) {
...
- (void)startTimer {
    if (self.showTime > 0) {
        HelloPopViewWK(self);;
        NSString *idStr = [NSString stringWithFormat:@"HelloPopView_%p",self];
        [HelloTimer addMinuteTimerForTime:self.showTime identifier:idStr handle:^(NSString * day, NSString * hour, NSString * minute, NSString * second, NSString * ms) {
...
/** 删除指定代理 */
- (void)removeForDelegate:(id<HelloPopViewProtocol>)delegate {
    if (delegate) {
...
- (void)statusBarOrientationChange:(NSNotification *)notification {
    if (self.isObserverScreenRotation) {
        ...
        self.frame = CGRectMake(0, 0, hl_ScreenWidth(), hl_ScreenHeight());
...
- (void)keyboardWillShow:(NSNotification *)notification{
    ...
    CGFloat customViewMaxY = self.customView.hl_bottom + self.avoidKeyboardSpace;
    ...
    //键盘遮挡到弹窗
    if ((keyboardMaxY<customViewMaxY) || ((_originFrame.origin.y + _customView.hl_height) > keyboardMaxY)) {
        //执行动画
        [UIView animateWithDuration:duration animations:^{
            self.customView.hl_y = self.customView.hl_y - self.avoidKeyboardOffset;
        }];
    }
}
...
- (void)keyboardWillHide:(NSNotification *)notification{
    ...
    CGFloat duration = [notification.userInfo[UIKeyboardAnimationDurationUserInfoKey] floatValue];
    [UIView animateWithDuration:duration animations:^{
        self.customView.hl_y = self.originFrame.origin.y;
    }];
}
...
- (void)pan:(UIPanGestureRecognizer *)panGestureRecognizer {
    ...
    self.panOffsetBlock?self.panOffsetBlock(CGPointMake(_customView.hl_x-_originFrame.origin.x, _customView.hl_y-_originFrame.origin.y)):nil;
    
    if (self.dragStyle == HelloDragStyleNO) {return;}
    // 获取手指的偏移量
    CGPoint transP = [panGestureRecognizer translationInView:self.customView];

    CGPoint velocity = [panGestureRecognizer velocityInView:hello_Window()];
    if(self.isDragScrollView) {//含有tableView,collectionView,scrollView
        ...
        if(self.scrollerView.contentOffset.y <= 0) {
            ...
            if(transP.y > 0) {
                ...
                self.isDragScrollView = NO;
                //向下拖
                self.customView.frame = CGRectMake(_customView.hl_x, _customView.hl_y + transP.y, _customView.hl_width, _customView.hl_height);
            } else {
                //如果向上拖拽
            }
        }
        ...
        CGFloat customViewX = self.customView.hl_x;
        CGFloat customViewY = self.customView.hl_y;
        //X正方向移动
        if ((self.dragStyle & HelloDragStyleX_Positive) && (customViewX >= _originFrame.origin.x)) {
            if (transP.x > 0) {
        ...
        //X负方向移动
        if ((self.dragStyle & HelloDragStyleX_Negative) && (customViewX <= self.originFrame.origin.x)) {
            if (transP.x < 0) {
                customViewX = customViewX + transP.x;
            } else if(transP.x > 0 && customViewX < _originFrame.origin.x ){
                customViewX = (customViewX + transP.x) < _originFrame.origin.x? (customViewX + transP.x):(_originFrame.origin.x);
        ...
        //Y正方向移动
        if (self.dragStyle & HelloDragStyleY_Positive && (customViewY >= _originFrame.origin.y)) {
            if (transP.y > 0) {
        ...
        //Y负方向移动
        if (self.dragStyle & HelloDragStyleY_Negative&&(customViewY <= _originFrame.origin.y)) {
            if (transP.y < 0) {
                customViewY = customViewY + transP.y;
            } else if(transP.y > 0 && customViewY < _originFrame.origin.y){
                customViewY = (customViewY + transP.y) < _originFrame.origin.y?(customViewY + transP.y):(_originFrame.origin.y);
        ...
        self.customView.frame = CGRectMake(customViewX, customViewY, _customView.hl_width, _customView.hl_height);
    }
    ...
    if(panGestureRecognizer.state == UIGestureRecognizerStateEnded) {
        ...
        void (^sweepBlock)(BOOL,BOOL,BOOL,BOOL) = ^(BOOL isX_P, BOOL isX_N, BOOL isY_P, BOOL isY_N) {
            ...
            if (isY_P==NO && isY_N==NO && self.sweepDismissStyle == HelloSweepDismissStyleSmooth) {//X轴可轻扫
                if (velocity.x>0) {//正向
                    [self dismissWithStyle:HelloDismissStyleSmoothToRight duration:self.dismissDuration];
                } else {//负向
                    [self dismissWithStyle:HelloDismissStyleSmoothToLeft duration:self.dismissDuration];
                }
                return;
            }
            
            if (isX_P==NO && isX_N==NO && self.sweepDismissStyle == HelloSweepDismissStyleSmooth) {//Y轴可轻扫
                if (velocity.y>0) {//正向
                    [self dismissWithStyle:HelloDismissStyleSmoothToBottom duration:self.dismissDuration];
                } else {//负向
                    [self dismissWithStyle:HelloDismissStyleSmoothToTop duration:self.dismissDuration];
                }
                return;
            }
            // 移除，以手势速度飞出
            [UIView animateWithDuration:0.5 animations:^{
                wk_self.backgroundView.backgroundColor = [self getNewColorWith:self.bgColor alpha:0.0];
                self.customView.center = CGPointMake(isX_P || isX_N?velocity.x:self.customView.hl_centerX, isY_P||isY_N?velocity.y:self.customView.hl_centerY);
            } completion:^(BOOL finished) {
                [wk_self dismissWithStyle:HelloDismissStyleFade duration:0.1];
            }];
        };
        ...
        if((velocityX >= self.swipeVelocity)||(velocityY >= self.swipeVelocity)) {//轻扫
            ...
            if ((self.dragStyle & HelloDragStyleX_Positive) && velocity.x>0 && velocityX >= self.swipeVelocity) {
                isX_P = self.sweepStyle & HelloSweepStyleX_Positive? YES:NO;
            }
            if ((self.dragStyle & HelloDragStyleX_Negative) && velocity.x<0 && velocityX >= self.swipeVelocity) {
                isX_N = self.sweepStyle & HelloSweepStyleX_Negative? YES:NO;
            }
            
            if ((self.dragStyle & HelloDragStyleY_Positive) && velocity.y>0 && velocityY >= self.swipeVelocity) {
                isY_P = self.sweepStyle & HelloSweepStyleY_Positive? YES:NO;
            }
            if ((self.dragStyle & HelloDragStyleY_Negative) && velocity.y <0 && velocityY >= self.swipeVelocity) {
                isY_N = self.sweepStyle & HelloSweepStyleY_Negative? YES:NO;
            }
...
//            LSTPVLog(@"isX=%@,isY=%@,velocityX=%lf,velocityY=%lf",isX?@"YES":@"NO",isY?@"YES":@"NO",velocityX,velocityY);
            ...
            if (self.dragStyle & HelloDragStyleAll) {
...
- (NSHashTable<id<HelloPopViewProtocol>> *)delegates {
...
/** 保存popView */
+ (void)savePopView:(HelloPopView *)popView {
    [HelloPopViewManager savePopView:popView];
}

/** 获取全局(整个app内)所有popView */
+ (NSArray *)getAllPopView {
    return [HelloPopViewManager getAllPopView];
}

/** 获取当前页面所有popView */
+ (NSArray *)getAllPopViewForParentView:(UIView *)parentView {
    return [HelloPopViewManager getAllPopViewForParentView:parentView];
}

/** 获取当前页面指定编队的所有popView */
+ (NSArray *)getAllPopViewForPopView:(HelloPopView *)popView {
    return [HelloPopViewManager getAllPopViewForPopView:popView];
}
 ...
 */
+ (HelloPopView *)getPopViewForKey:(NSString *)key {
    return [HelloPopViewManager getPopViewForKey:key];
}

/** 移除popView */
+ (void)removePopView:(HelloPopView *)popView {
    [HelloPopViewManager removePopView:popView];
}

+ (void)removePopView:(HelloPopView *)popView complete:(Hello_Block_Void)complete
{
    if (popView == nil) {
        complete ? complete() : nil;
    }
...
+ (void)removePopViewForKey:(NSString *)key {
    [HelloPopViewManager removePopViewForKey:key complete:nil];
}

+ (void)removePopViewForKey:(NSString *)key complete:(Hello_Block_Void)complete
{
    [HelloPopViewManager removePopViewForKey:key complete:complete];
}

/** 移除所有popView */
+ (void)removeAllPopView {
    [HelloPopViewManager removeAllPopViewComplete:nil];
}

+ (void)removeAllPopViewComplete:(Hello_Block_Void)complete
{
    [HelloPopViewManager removeAllPopViewComplete:complete];
}

/** 移除 最后一个弹出的 popView */
+ (void)removeLastPopView {
    [HelloPopViewManager removeLastPopViewComplete:nil];
}

+ (void)removeLastPopViewComplete:(Hello_Block_Void)complete
{
    [HelloPopViewManager removeLastPopViewComplete:complete];
}

/** 开启调试view  建议设置成 线上隐藏 测试打开 */
+ (void)setLogStyle:(HelloPopViewLogStyle)logStyle {
    _logStyle = logStyle;
    
    if (logStyle == HelloPopViewLogStyleNO) {
        [HelloPopViewM().infoView removeFromSuperview];
        HelloPopViewM().infoView = nil;
    }
}
